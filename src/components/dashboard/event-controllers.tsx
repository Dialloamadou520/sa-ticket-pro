...
import type { EventController } from "@/lib/types";
...
export function EventControllers({ eventId, controllers, scanCounts }: Props) {
  ...
  function onRemove(id: string) {
    ...
    startTransition(async () => {
      await removeController(eventId, id);
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="mb-3 text-xs text-slate-400">
          Saisissez l&apos;email du contrôleur. Il pourra scanner les tickets de
          cet événement après s&apos;être connecté (ou inscrit) avec cet email.
        </p>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white">
        ...
        {controllers.length === 0 ? (
          <p className="px-5 py-10 text-center text-sm text-slate-500">
            Aucun contrôleur pour le moment.
          </p>
        ) : (
            ...
            {controllers.map((c) => {
              ...
              >
                ...
                <button
                  ...
                  aria-label="Retirer le contrôleur"
                >
                  {pendingId === c.id ? (
